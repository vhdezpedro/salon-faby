...
export const create = async (req, res) => {
  try {
    const { name, date, time, service_id } = req.body;
    if (!name || !date || !time || !service_id) {
      return res.status(400).json({ error: "Todos los campos son requeridos" });
    }
    const appointment = await Appointment.create(name, date, time, service_id);
...
export const update = async (req, res) => {
  try {
    ...
    const appointment = await Appointment.update(req.params.id, name, date, time, service_id);
    res.json(appointment);
  } catch (error) {
